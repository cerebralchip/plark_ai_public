...
class PlarkEnv(gym.Env):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        self.random_panther_start_position = kwargs.get(
            "random_panther_start_position", False
        )
        ...
        if self.image_based:
            ...
            logger.info(
                ...
                + str(self.render_height)
                + ", Width:"
                + str(self.render_width)
                + ", Channels:"
                + str(N_CHANNELS)
            )
            self.observation_space = spaces.Box(
                low=0,
                high=255,
                shape=(self.render_height, self.render_width, N_CHANNELS),
                dtype=np.uint8,
            )
            ...
            if len(self.env.activeGames) > 0:
                ...
                if self.config_file_path:
                    ...
                    self.env.createNewGame(
                        config_file_path=self.config_file_path, **self.kwargs
                    )
                else:
    ...
    def _observation(self):
        if self.image_based:
            pil_image = self.env.activeGames[len(self.env.activeGames) - 1].render(
                self.render_width, self.render_height, self.view
            )
            np_image = np.array(pil_image, dtype=np.uint8)
            return np_image
        else:
            obs = self.observation.get_observation(
                self.env.activeGames[len(self.env.activeGames) - 1]._state(self.view)
            )
            # return obs
    ...
    def step(self, action):
        ...
        if (
            self.status == "PELICANWIN"
            or self.status == "BINGO"
            or self.status == "WINCHESTER"
            or self.status == "ESCAPE"
        ):
    ...
    def reset(self):
        ...
        if len(self.env.activeGames) > 0:
            if (
                self.random_panther_start_position
            ):  # On reset randomy places panther in a different location.
                map_width = self.env.activeGames[
                    len(self.env.activeGames) - 1
                ].map_width
                map_height = self.env.activeGames[
                    len(self.env.activeGames) - 1
                ].map_height
                panther_start_col = random.choice(range(0, map_width - 1))
                panther_start_row = random.choice(range(map_height - 4, map_height - 1))
                self.env.activeGames[
                    len(self.env.activeGames) - 1
                ].panther_start_col = panther_start_col
                self.env.activeGames[
                    len(self.env.activeGames) - 1
                ].panther_start_row = panther_start_row
            self.env.activeGames[len(self.env.activeGames) - 1].reset_game()
        else:
            if self.config_file_path:
                logger.info("config filepath: " + str(self.config_file_path))
    ...
    def render(self, mode="human", close=False, view=None):
        ...
        view = "ALL"
        pil_image = self.env.activeGames[len(self.env.activeGames) - 1].render(
            self.render_width, self.render_height, view
        )
        return pil_image
